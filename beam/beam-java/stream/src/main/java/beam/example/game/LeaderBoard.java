...
import beam.example.game.utils.WriteToBigQuery;
...
public class LeaderBoard extends HourlyTeamScore {
   ...
   */
  public interface Options extends ExampleOptions, StreamingOptions, HourlyTeamScore.Options, DataflowPipelineWorkerPoolOptions {
  ...
  static class WriteLogToPubsub extends Thread {
    String[] args;

    public WriteLogToPubsub(String[] args) {
      this.args = args;
    }

    @SneakyThrows
    public void run() {
      Options options = PipelineOptionsFactory.fromArgs(ExampleUtils.appendArgs(args)).as(Options.class);
      options.setJobName("game-publishing");
      options.setRunner(PIPELINE_RUNNER);
      options.setMaxNumWorkers(MAX_NUM_WORKER);
      Pipeline pipeline = Pipeline.create(options);

      System.out.println("Start publishing message to " + TOPIC_NAME.toString());
      pipeline
        .apply(TextIO.read().from(INPUT_FILE))
        .apply(PubsubIO.writeStrings().withTimestampAttribute(GameConstants.TIMESTAMP_ATTRIBUTE).to(TOPIC_NAME.toString()));

      pipeline.run();
      System.out.println("Finish publishing message to " + TOPIC_NAME.toString());
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    Options options = PipelineOptionsFactory.fromArgs(ExampleUtils.appendArgs(args)).as(Options.class);
    options.setJobName("game-streaming");
