...
var (
	...
	filter = flag.String("filter", "Flourish|stomach|stay", "Regex filter pattern to use. Only words matching this pattern will be included.")
	output = flag.String("output", "", "Output file (required).")
)

var (
	filteredLen = beam.NewDistribution("extract", "filtered match statistics")
)
...
func (f *filterFn) Setup() {
	fmt.Println("Setup filter fn with regex >> ", f.Filter)
...
func (f *filterFn) ProcessElement(ctx context.Context, word string, count int, emit func(string, int)) {
	if f.re.MatchString(word) {
		...
		beamLog.Infof(ctx, "Matched: %v", word)

		filteredLen.Update(ctx, int64(count))

		emit(word, count)
	} else {
		// Log at the "DEBUG" level each element that is not matched.
		beamLog.Debugf(ctx, "Did not match: %v", word)
	}
}
...
func main() {
	...
	if *output == "" {
		beamLog.Exit(ctx, "No output provided")
	}
	if _, err := regexp.Compile(*filter); err != nil {
		beamLog.Exitf(ctx, "Invalid filter: %v", err)
	}
	...
	passert.Equals(s, formatted, "Flourish: 3", "stomach: 1", "stay: 7")

	if err := beamx.Run(ctx, p); err != nil {
		beamLog.Exitf(ctx, "Failed to execute job: %v", err)
	}
}
