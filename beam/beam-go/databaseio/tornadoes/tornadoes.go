...
// 1. Create database and table on Postgresql based on the dsn
// CREATE DATABASE beam_tutorial;
// USE beam_tutorial;

// 2. CREATE TABLE IF NOT EXISTS tornadoes (
//      month INT NOT NULL,
//      tornado_count INT NOT NULL
//    );

var (
	input = flag.String("input", "clouddataflow-readonly:samples.weather_stations",
		"BigQuery table with weather data to read from, specified as <project_id>:<dataset_id>.<table_id>")

	driver = flag.String("driver", "mysql", "Database driver")
	dsn    = flag.String(
		"dsn",
		"root:password@tcp(localhost:3306)/beam_tutorial",
		"Database dsn",
	)
	table = flag.String("table", "tornadoes", "Output table name")
)
...
type TornadoRow struct {
	Month Month `pg:"month" column:"month"`
	Count int   `pg:"tornado_count" column:"tornado_count"`
}
...
func main() {
	...
	databaseio.WriteWithBatchSize(s, 1, *driver, *dsn, *table, []string{
		"month",
		"tornado_count",
	}, out)
