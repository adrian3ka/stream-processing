...
type filterFn struct {
	...
	MinimumWordCount uint64 `json:"minimum_word_count"`
}

func (f *filterFn) Setup() {
	fmt.Println("Setup filter fn with minimum word count >> ", f.MinimumWordCount)
}
...
func (f *filterFn) ProcessElement(ctx context.Context, word string, count int, emit func(string, int)) {
	if uint64(count) > f.MinimumWordCount {
		// Log at the "INFO" level each element that we match.
		beamLog.Infof(ctx, "Meet miniumum word count : %v - %d ", word, count)
		...
		beamLog.Debugf(ctx, "Did not meet minimum word count : %v - %d", word, count)
	}
}
...
func main() {
	...
	filtered := beam.ParDo(s, &filterFn{MinimumWordCount: 1}, counted)
