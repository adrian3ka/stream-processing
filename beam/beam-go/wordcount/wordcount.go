...
var (
	...
	outputUnique = flag.String("outputUnique", "", "Output file (required).")
)
...
var (
	...
	empty   = beam.NewCounter("extract", "empty lines")
	lineLen = beam.NewDistribution("extract", "line statistics")
	wordLen = beam.NewDistribution("extract", "word per line statistics")

	// Give information last time the entity is called, so it would be the latest line information
	lastLineInformation = beam.NewGauge("extract", "last line word length")
)
...
func extractFn(ctx context.Context, line string, emit func(string)) {
	...
	fmt.Println("extractFn", line)

	lineLen.Update(ctx, int64(len(line)))
	wordLen.Update(ctx, int64(len(words)))
	lastLineInformation.Set(ctx, int64(len(words)))
...
func formatFn(w string, c int) string {
	fmt.Println("formatFn ", w, c)
	return fmt.Sprintf("%s: %v", w, c)
}

// formatFnUnique is a DoFn that formats a word and its count as a string.
func formatFnUnique(w string, _ int) string {
	s := fmt.Sprint("Unique >> ", w)
	fmt.Println(s)
...
// extractUniqueWordsFn is a DoFn that emits the words in a given line.
func extractUniqueWordsFn(ctx context.Context, line string, emit func(string)) {
	fmt.Println("extractUniqueWordsFn", line)
...
func CountUniqueWords(s beam.Scope, lines beam.PCollection) beam.PCollection {
	s = s.Scope("CountUniqueWords")

	// Convert lines of text into individual words.
	col := beam.ParDo(s, extractUniqueWordsFn, lines)
...
func main() {
	...
	// Input validation is done as usual. Note that it must be after Init().
	if *outputUnique == "" {
		log.Fatal("No outputUnique provided")
	}
	...
	// Output from countedWords will be processed here
	countedUniqueWords := CountUniqueWords(s, formatted)
	formattedUniqueWords := beam.ParDo(s, formatFnUnique, countedUniqueWords)

	textio.Write(s, *output, formatted)
	textio.Write(s, *outputUnique, formattedUniqueWords)

	fmt.Println("Starting Beam....")
