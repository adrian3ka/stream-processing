...
// 2. Create datasets name cookbook

// 3. Run the following SQL :
//   CREATE TABLE `cookbook.combine_output` (
//     word STRING OPTIONS(description="Word informations"),
//     plays STRING OPTIONS(description="PLays informations")
//   );

// 4. pin project `bigquery-public-data` to see the example data

var (
	input  = flag.String("input", "bigquery-public-data:samples.shakespeare", "Shakespeare plays BQ table.")
	output = flag.String("output", "beam-tutorial-272917:cookbook.combine_output", "Output BQ table.")
...
func PlaysForWords(s beam.Scope, rows beam.PCollection) beam.PCollection {
	fmt.Println("Setup PlaysForWords pipeline .... ")
...
func (f *extractFn) ProcessElement(row WordRow, emit func(string, string)) {
	reachedMinimumLength := len(row.Word) >= f.MinLength

	fmt.Println("-----------------------------------------------------------------")
	fmt.Println("row word >> ", row.Word)
	fmt.Println("len row word >>", len(row.Word), "| reachedMinimumLength >>", reachedMinimumLength)
	fmt.Println("row corpus >> ", row.Corpus)

	if reachedMinimumLength {
...
func concatFn(a, b string) string {
	concattedString := fmt.Sprintf("%v,%v", a, b)
	fmt.Println("concattedString >>", concattedString)
	return concattedString
}

func formatFn(word, plays string) PlaysRow {
	fmt.Printf("words : %s | plays : %s\n", word, plays)
